...
import {
  setCreateUserFirstStep,
  setCreateUserSecondStep,
} from "../../../redux/features/createUserDataHandleSlice/createUserDataHandleSlice";

const SetUserPassword = ({ step, setStep }) => {
  const { createUserFirstStep, createUserSecondStep } = useSelector(
  ...
  const onSubmit = (data) => {
    ...
    const user = {
      ...
      status: "Active",
      roll: "User",
      openingDateISO: new Date().toISOString(),
    };
    const payload = { user, label };
    axios
      .post(
        `https://dream-records-2025-m2m9a.ondigitalocean.app/admin/api/v1/users/create-user`,
        payload
      )
      .then((res) => {
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="createUser-form">
        ...
        <div className="createUser-btns">
          ...
          {loading ? (
            <button disabled className="theme-btn">
              loading...
            </button>
          ) : (
